...
class PaginationFiltreService{
    ...
    public function getData(): array
    {
        ...
        if(!$this->search && !$this->filtre){
            ...
            return $this->manager
            ->getRepository($this->entityClass)
            ->search("",$this->filtre,$this->limit,$offset);
        }else{
            return $this->manager
                        ->getRepository($this->entityClass)
                        ->search($this->search,$this->filtre,$this->limit,$offset);
        }
    }
    ...
    public function getPages(): int
    {
        ...
        if(!$this->search && !$this->filtre){
            ...
            $total = count($this->manager->getRepository($this->entityClass)->search($this->search,$this->filtre));
        }
        if($total === 0){
            $total = $total+1;
        }
