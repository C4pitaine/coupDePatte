...
class PaginationTypeService{
    ...
    /**
     * Variable qui récupère le type d'animal
     *
     * @var string
     */
    private string $type;
    ...
    public function getData(): array
    {
        ...
        if(!$this->search && !$this->filtre){
            return $this->manager
                        ...
                        ->type("","",$this->type,$this->limit,$offset);
        }else if($this->filtre && !$this->search){
            return $this->manager
            ->getRepository($this->entityClass)
            ->type("",$this->filtre,$this->type,$this->limit,$offset);
        }else{
            return $this->manager
                        ->getRepository($this->entityClass)
                        ->type($this->search,$this->filtre,$this->type,$this->limit,$offset);
        }
    }
    ...
    public function getPages(): int
    {
        ...
        if(!$this->search && !$this->filtre){
            ...
            $total = count($this->manager->getRepository($this->entityClass)->type($this->search,$this->filtre,$this->type));
        }
        if($total === 0){
            $total = $total+1;
        }
